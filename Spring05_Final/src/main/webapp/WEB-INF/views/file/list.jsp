		...
		<tbody>
			<c:forEach var="tmp" items="${list }">
				<tr>
					...
					<td>${tmp.writer }</td>
					<td>${tmp.title }</td>
					<td><a href="download.do?num=${tmp.num}" >
					${tmp.orgFileName}</td>
					<td><fmt:formatNumber value="${tmp.fileSize}" pattern="#,###"/></td>
					<td>${tmp.downCount}</td>
					<td>${tmp.regdate}</td>
					<td>
						<%--글 작성자하고 session에 저장된 id와 같을 때만 삭제 링크를 제공하겠다는 의미 --%>
		...
		<ul class="pagination pagination-sm">
			<c:choose>
				<c:when test="${startPageNum ne 1 }">
					<li>
						<a href="list.do?pageNum=${starPageNum-1}&condition=${condition}&keyword=${encodedKeyword}">&laquo;</a>
					</li>
				</c:when>
				<c:otherwise>
					<li class="disabled">
